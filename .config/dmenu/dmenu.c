...
//	unsigned int curpos;
	...
	if (passwd)
		...
		curpos = MIN(curpos - TEXTW(numbers), curlen);
	oldcurlen = curlen;
	...
	if (lines > 0) {
		...
		for (item = curr; item != next; item = item->right) {
			if (indent)
				drawitem(item, x + curpos - 10, y += bh, mw - x);
			else
				drawitem(item, x - 10, y += bh, mw - x);
		}
	} else if (matches) {
		...
		x += w;
		for (item = curr; item != next; item = item->right) {
			/* don't show numbers when password mode is enabled */
			if (passwd)
...
keypress(XKeyEvent *ev)
{
	...
	if (ev->state & ControlMask) {
		...
		case XK_l: ksym = XK_Delete;    break;
	...
	//		break;
		...
		case XK_Y:
			...
			return;
		case XK_v:/* clip = "clipboard"(XA_CLIPBOARD) | XA_PRIMARY == "primary" */
